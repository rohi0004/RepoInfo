...
const Feature = ({ icon: Icon, title, description, gradient, delay }: FeatureProps) => {
    return (
        ...
        >
            ...
            {/* Card */}
            <div className="relative h-full rounded-xl p-6 transition-all duration-300" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
                {/* Icon container */}
                    <div className={`w-14 h-14 rounded-lg ${gradient} p-0.5 mb-4`}>
                    <div className="w-full h-full rounded-lg flex items-center justify-center" style={{ background: 'var(--surface)' }}>
                        <Icon className="w-6 h-6" style={{ color: 'var(--foreground)' }} />
                    </div>
                </div>

                {/* Content */}
                <h3 className="text-xl font-semibold mb-2 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-zinc-400 transition-all duration-300" style={{ color: 'var(--foreground)' }}>
                    {title}
                </h3>
