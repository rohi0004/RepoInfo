    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center h-screen gap-4" style={{ background: 'var(--background)', color: 'var(--foreground)' }}>
                <UserIcon className="w-16 h-16" style={{ color: 'var(--accent)' }} />
                    ...
                    <div className="mb-8 text-center">
                        ...
                        >
                            <img
                                ...
                                alt={username}
                                className="w-24 h-24 rounded-2xl border-4 shadow-xl"
                                style={{ borderColor: 'var(--border)' }}
                            />
                        </motion.div>
                        <h2 className="text-2xl font-bold mb-2">Loading @{username}</h2>
                        <p className="text-sm" style={{ color: 'var(--muted)' }}>Analyzing profile and repositories...</p>
                    </div>
                    ...
                    <div className="space-y-3">
                        {steps.map((step, index) => (
                            ...
                            >
                                ...
                                {step.status === "error" && (
                                    <FileCode className="w-5 h-5 shrink-0" style={{ color: '#ef4444' }} />
                                )}
                                <span className="text-sm" style={{ color: 'var(--muted)' }}>{step.message}</span>
                            </motion.div>
                        ))}
                    </div>
                </div>
